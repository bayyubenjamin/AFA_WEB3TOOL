...
export default function PageLoginWithTelegram() {
    ...
    useEffect(() => {
        // Fungsi ini akan mencoba menginisialisasi aplikasi, dengan mencoba ulang beberapa kali jika objek Telegram WebApp belum siap.
        const initializeWithRetries = (attempt = 1) => {
            const tg = window.Telegram?.WebApp;

            // Periksa apakah data pengguna tersedia.
            if (tg && tg.initDataUnsafe?.user?.id) {
                tg.ready(); // Beri tahu Telegram bahwa aplikasi siap untuk ditampilkan.
                
                const tgUser = tg.initDataUnsafe.user;
                setTelegramUser(tgUser);
                setUsername(tgUser.username || `user${tgUser.id}`);
                setStatus('ready'); // Atur status menjadi 'ready' untuk merender form.
                return; // Keluar dengan sukses.
            }

            // Jika data belum siap, dan kita belum melebihi batas percobaan ulang, tunggu dan coba lagi.
            if (attempt <= 10) { // Coba hingga 10 kali (total 1 detik)
                setTimeout(() => initializeWithRetries(attempt + 1), 100);
            } else {
                // Jika masih belum siap setelah semua percobaan, tampilkan pesan error.
                setStatus('error');
                setError('Failed to initialize Telegram data. Please try reopening the Mini App.');
            }
        };

        // Pemeriksaan awal untuk memastikan kita berada di dalam lingkungan Telegram.
    ...
    }, []); // Dependency array kosong memastikan ini hanya berjalan sekali saat komponen dimuat.
