...
export default function PageProfile({ currentUser, onUpdateUser, userAirdrops = [], navigateTo }) {
  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      setError(err.message || (t.loginFailed || "Gagal login."));
    } finally { setLoading(false); }
  };

  const handleSignupRequestOtp = async (e) => {
    e.preventDefault(); clearMessages(); setLoading(true);
    if (!signupUsername || !signupEmail || !signupPassword) { setError(t.allFieldsRequired || "Username, Email, dan Password harus diisi!"); setLoading(false); return; }
    if (signupPassword !== signupConfirmPassword) { setError(t.passwordsMismatch || "Password tidak cocok!"); setLoading(false); return; }
    try {
      ...
      setSuccessMessage((t.otpSentTo || `Kode OTP telah dikirim ke`) + ` ${signupEmail}.`);
      setSignupStage('awaitingOtp');
    } catch (err) { setError(err.message || (t.failedToSendOtp || "Gagal mengirim OTP.")); }
    finally { setLoading(false); }
  };
  ...
  const handleVerifyOtpAndCompleteSignup = async (e) => {
    ...
    try {
      ...
      setSuccessMessage(t.signupSuccess || "Pendaftaran berhasil! Anda akan login secara otomatis.");
    } catch (err) {
      setError(err.message || (t.otpVerificationFailed || "Verifikasi OTP atau pembuatan profil gagal."));
    } finally { setLoading(false); }
  };
  ...
  const handleUpdateProfile = async (e) => {
    ...
    } catch (err) { setError(err.message || (t.profileUpdateFailed || "Gagal update profil."));
    } finally { setLoading(false); }
  };
  ...
  if (!currentUser) {
    return (<section className="page-content text-center pt-20"><FontAwesomeIcon icon={faSpinner} spin size="2x" className="text-primary"/><p>{commonT.loadingApp}</p></section>);
  }
          ...
          <div className="card rounded-xl p-6 md:p-8 shadow-2xl">
            <div className="text-center mb-6">
              ...
              <p className="text-gray-400 mt-2">
                {isLoginForm ? (t.loginToContinue || "Login untuk melanjutkan.") : (signupStage === 'collectingDetails' ? (t.fillSignupDetails || "Isi data untuk mendaftar.") : (t.verifyAccount || "Verifikasi akun Anda."))}
              </p>
            </div>
            {isLoginForm ? (
              <form onSubmit={handleLogin} className="space-y-4">
                <InputField language={language} parentLoading={loading} id="loginEmail" type="email" label={t.emailLabel} value={loginEmail} onChange={(e) => setLoginEmail(e.target.value)} icon={faEnvelope} placeholder={t.emailPlaceholder} />
                <div className="relative">
                    <InputField language={language} parentLoading={loading} id="loginPassword" type={showPassword ? "text" : "password"} label={t.passwordLabel} value={loginPassword} onChange={(e) => setLoginPassword(e.target.value)} icon={faLock} placeholder={t.passwordPlaceholder} />
                ...
                <button type="submit" disabled={loading} className="btn-primary text-white font-semibold py-3 px-8 rounded-lg text-lg w-full flex items-center justify-center disabled:opacity-70">
                  {loading ? <FontAwesomeIcon icon={faSpinner} spin className="mr-2" /> : <FontAwesomeIcon icon={faSignInAlt} className="mr-2" />} {t.loginButton}
                </button>
              </form>
            ) : (
              <>
                {signupStage === 'collectingDetails' ? (
                  <form onSubmit={handleSignupRequestOtp} className="space-y-4">
                    <InputField language={language} parentLoading={loading} id="signupUsername" label={t.usernameLabel} value={signupUsername} onChange={(e) => setSignupUsername(e.target.value)} icon={faUser} placeholder={t.usernamePlaceholder} />
                    <InputField language={language} parentLoading={loading} id="signupEmail" type="email" label={t.emailLabel} value={signupEmail} onChange={(e) => setSignupEmail(e.target.value)} icon={faEnvelope} placeholder={t.emailPlaceholder} />
                    <div className="relative">
                        <InputField language={language} parentLoading={loading} id="signupPassword" type={showPassword ? "text" : "password"} label={t.passwordLabel} value={signupPassword} onChange={(e) => setSignupPassword(e.target.value)} icon={faLock} placeholder={t.minCharPassword} />
                    ...
                    <div className="relative">
                        <InputField language={language} parentLoading={loading} id="signupConfirmPassword" type={showConfirmPassword ? "text" : "password"} label={t.confirmPasswordLabel} value={signupConfirmPassword} onChange={(e) => setSignupConfirmPassword(e.target.value)} icon={faLock} placeholder={t.repeatPasswordPlaceholder} />
                    ...
                    <button type="submit" disabled={loading} className="btn-primary text-white font-semibold py-3 px-8 rounded-lg text-lg w-full flex items-center justify-center disabled:opacity-70">
                      {loading ? <FontAwesomeIcon icon={faSpinner} spin className="mr-2" /> : <FontAwesomeIcon icon={faUserPlus} className="mr-2" />} {t.signupButton}
                    </button>
                  </form>
                ) : (
                  <form onSubmit={handleVerifyOtpAndCompleteSignup} className="space-y-4">
                    <InputField language={language} parentLoading={loading} id="otpCode" type="text" label={t.otpCodeLabel} value={otpCode} onChange={(e) => setOtpCode(e.target.value)} icon={faKey} placeholder={t.otpPlaceholder} />
                    <button type="submit" disabled={loading} className="btn-primary text-white font-semibold py-3 px-8 rounded-lg text-lg w-full flex items-center justify-center disabled:opacity-70">
                      {loading ? <FontAwesomeIcon icon={faSpinner} spin className="mr-2" /> : <FontAwesomeIcon icon={faUserPlus} className="mr-2" />} {t.verifyAndCompleteButton}
                    </button>
                    <button type="button" onClick={handleBackToDetails} disabled={loading} className="text-center w-full text-sm text-gray-400 hover:text-primary disabled:opacity-50">{t.wrongEmailBack || "Salah email? Kembali"}</button>
                  </form>
                )}
              </>
            )}
            <p className="text-center text-sm text-gray-400 mt-6">
              {isLoginForm ? (t.noAccountYet || "Belum punya akun?") : (t.alreadyHaveAccount || "Sudah punya akun?")}{" "}
              <button disabled={loading} onClick={() => { setIsLoginForm(!isLoginForm); clearMessages(); setSignupStage('collectingDetails'); }} className="font-semibold text-primary hover:underline disabled:opacity-50">
                {isLoginForm ? (t.signupHere || "Daftar di sini") : (t.loginHere || "Login di sini")}
              </button>
            </p>
          </div>
        </div>
        ...
        <>
          <div className="card rounded-xl p-6 md:p-8 shadow-xl">
            <div className="flex flex-col sm:flex-row items-center text-center sm:text-left gap-5 md:gap-8">
              <div className="relative group">
                ...
                <button onClick={handleOpenEditProfileModal} className="absolute inset-0 w-full h-full bg-black/50 rounded-full flex items-center justify-center text-white text-2xl opacity-0 group-hover:opacity-100 transition-opacity" aria-label={t.editAvatar || "Edit Avatar"} disabled={loading}><FontAwesomeIcon icon={faEdit} /></button>
              </div>
              <div className="flex-grow">
                ...
                <div className="mt-4 space-x-3">
                    <button disabled={loading} onClick={handleOpenEditProfileModal} className="btn-secondary text-xs px-5 py-2 rounded-lg inline-flex items-center" ><FontAwesomeIcon icon={faEdit} className="mr-1.5"/> {t.editProfileButton}</button>
                </div>
              </div>
            </div>
          </div>
          <div className="card rounded-xl p-6 md:p-8 shadow-xl">
            <h3 className="text-xl md:text-2xl font-semibold mb-5 text-primary border-b border-white/10 pb-3 flex items-center"><FontAwesomeIcon icon={faRobot} className="mr-2.5" /> {t.myStatisticsTitle}</h3>
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4 md:gap-5 text-sm">
              {[
                { label: t.questPointsLabel, value: currentUser.stats?.points || 0 },
                { label: t.airdropsClaimedLabel, value: currentUser.stats?.airdropsClaimed || 0 },
                { label: t.nftsOwnedLabel, value: currentUser.stats?.nftsOwned || 0 },
                { label: t.activeAirdropsLabel, value: activeAirdropsCount }
          ...
          <div className="mt-6 md:mt-8">
            <button disabled={loading} onClick={handleLogout} className="w-full btn-danger bg-red-600/80 hover:bg-red-700/90 text-white font-semibold py-3.5 px-6 rounded-lg flex items-center justify-center text-base">
              {loading ? <FontAwesomeIcon icon={faSpinner} spin className="mr-2.5" /> : <FontAwesomeIcon icon={faSignOutAlt} className="mr-2.5" />} {t.logoutButton}
            </button>
          </div>
        </>
      )}
        ...
        <div className="fixed inset-0 bg-black/70 backdrop-blur-sm flex items-center justify-center p-4 z-[100]">
          <div className="modal-content card rounded-xl p-6 md:p-8 shadow-2xl w-full max-w-lg">
            ...
            <form onSubmit={handleUpdateProfile} className="space-y-5">
              <InputField language={language} parentLoading={loading} id="editName" label={t.displayNameLabel} value={editName} onChange={(e) => setEditName(e.target.value)} icon={faUser} />
              <InputField language={language} parentLoading={loading} id="editAvatarUrl" label={t.avatarUrlLabel} value={editAvatarUrl} onChange={(e) => setEditAvatarUrl(e.target.value)} icon={faImage} />
              <div className="flex justify-end gap-4 pt-4">
                <button disabled={loading} type="button" onClick={handleCloseEditProfileModal} className="btn-secondary px-6 py-2.5 rounded-lg text-sm">{t.cancel}</button>
                <button disabled={loading} type="submit" className="btn-primary text-white px-6 py-2.5 rounded-lg text-sm flex items-center">
                  {loading ? <FontAwesomeIcon icon={faSpinner} spin className="mr-2" /> : <FontAwesomeIcon icon={faSave} className="mr-2" />} {t.saveChangesButton}
                </button>
              </div>
            </form>
          </div>
