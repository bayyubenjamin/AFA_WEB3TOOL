// src/components/PageProfile.jsx - VERSI FINAL LENGKAP
...
// InputField dipindah ke luar komponen PageProfile agar stabil
...
export default function PageProfile({ currentUser, onUpdateUser, userAirdrops = [], navigateTo }) {
  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      // onAuthStateChange di App.jsx akan handle sisanya
    } catch (err) {
  ...
  const handleSignupRequestOtp = async (e) => {
    e.preventDefault(); clearMessages(); setLoading(true);
    if (!signupUsername || !signupEmail || !signupPassword) { setError("Username, Email, dan Password harus diisi!"); setLoading(false); return; }
    if (signupPassword !== signupConfirmPassword) { setError("Password tidak cocok!"); setLoading(false); return; }
    try {
      ...
      setSuccessMessage(`Kode OTP telah dikirim ke ${signupEmail}.`);
      setSignupStage('awaitingOtp');
    } catch (err) { setError(err.message || "Gagal mengirim OTP."); } 
    finally { setLoading(false); }
  };

  const handleVerifyOtpAndCompleteSignup = async (e) => {
    e.preventDefault(); clearMessages(); setLoading(true);
    if (!otpCode) { setError("Kode OTP harus diisi!"); setLoading(false); return; }
    try {
      const { data: { session }, error: otpError } = await supabase.auth.verifyOtp({
        email: signupEmail, token: otpCode, type: 'signup',
      });
      if (otpError) throw otpError;
      if (!session?.user) throw new Error("Sesi tidak ditemukan setelah verifikasi OTP.");
      ...
      setSuccessMessage("Pendaftaran berhasil! Anda akan login secara otomatis.");
      // onAuthStateChange di App.jsx akan menangani sisanya
    } catch (err) {
  ...
  const handleUpdateProfile = async (e) => {
    ...
    } catch (err) { setError(err.message || "Gagal update profil.");
    } finally { setLoading(false); }
  };
  ...
  if (!currentUser) { 
    return (<section className="page-content text-center pt-20"><FontAwesomeIcon icon={faSpinner} spin size="2x" className="text-primary"/></section>);
  }
          ...
          <div className="card rounded-xl p-6 md:p-8 shadow-2xl">
            <div className="text-center mb-6">
              ...
              <p className="text-gray-400 mt-2">
                {isLoginForm ? "Login untuk melanjutkan." : (signupStage === 'collectingDetails' ? "Isi data untuk mendaftar." : "Verifikasi akun Anda.")}
              </p>
            </div>
            {isLoginForm ? (
              <form onSubmit={handleLogin} className="space-y-4">
                <InputField parentLoading={loading} id="loginEmail" type="email" label="Email" value={loginEmail} onChange={(e) => setLoginEmail(e.target.value)} icon={faEnvelope} placeholder="email@example.com" />
                <div className="relative">
                    <InputField parentLoading={loading} id="loginPassword" type={showPassword ? "text" : "password"} label="Password" value={loginPassword} onChange={(e) => setLoginPassword(e.target.value)} icon={faLock} placeholder="Password Anda" />
                ...
                <button type="submit" disabled={loading} className="btn-primary text-white font-semibold py-3 px-8 rounded-lg text-lg w-full flex items-center justify-center disabled:opacity-70">
                  {loading ? <FontAwesomeIcon icon={faSpinner} spin className="mr-2" /> : <FontAwesomeIcon icon={faSignInAlt} className="mr-2" />} Login
                </button>
              </form>
            ) : (
              <>
                {signupStage === 'collectingDetails' ? (
                  <form onSubmit={handleSignupRequestOtp} className="space-y-4">
                    <InputField parentLoading={loading} id="signupUsername" label="Username" value={signupUsername} onChange={(e) => setSignupUsername(e.target.value)} icon={faUser} placeholder="Pilih username unik" />
                    <InputField parentLoading={loading} id="signupEmail" type="email" label="Email" value={signupEmail} onChange={(e) => setSignupEmail(e.target.value)} icon={faEnvelope} placeholder="email@example.com" />
                    <div className="relative">
                        <InputField parentLoading={loading} id="signupPassword" type={showPassword ? "text" : "password"} label="Password" value={signupPassword} onChange={(e) => setSignupPassword(e.target.value)} icon={faLock} placeholder="Minimal 6 karakter" />
                    ...
                    <div className="relative">
                        <InputField parentLoading={loading} id="signupConfirmPassword" type={showConfirmPassword ? "text" : "password"} label="Konfirmasi Password" value={signupConfirmPassword} onChange={(e) => setSignupConfirmPassword(e.target.value)} icon={faLock} placeholder="Ulangi password" />
                    ...
                    <button type="submit" disabled={loading} className="btn-primary text-white font-semibold py-3 px-8 rounded-lg text-lg w-full flex items-center justify-center disabled:opacity-70">
                      {loading ? <FontAwesomeIcon icon={faSpinner} spin className="mr-2" /> : <FontAwesomeIcon icon={faUserPlus} className="mr-2" />} Daftar & Kirim OTP
                    </button>
                  </form>
                ) : (
                  <form onSubmit={handleVerifyOtpAndCompleteSignup} className="space-y-4">
                    <InputField parentLoading={loading} id="otpCode" type="text" label="Kode OTP" value={otpCode} onChange={(e) => setOtpCode(e.target.value)} icon={faKey} placeholder="Masukkan 6 digit kode" />
                    <button type="submit" disabled={loading} className="btn-primary text-white font-semibold py-3 px-8 rounded-lg text-lg w-full flex items-center justify-center disabled:opacity-70">
                      {loading ? <FontAwesomeIcon icon={faSpinner} spin className="mr-2" /> : <FontAwesomeIcon icon={faUserPlus} className="mr-2" />} Verifikasi & Selesaikan
                    </button>
                    <button type="button" onClick={handleBackToDetails} disabled={loading} className="text-center w-full text-sm text-gray-400 hover:text-primary disabled:opacity-50">Salah email? Kembali</button>
                  </form>
                )}
              </>
            )}
        ...
        <>
          <div className="card rounded-xl p-6 md:p-8 shadow-xl">
            <div className="flex flex-col sm:flex-row items-center text-center sm:text-left gap-5 md:gap-8">
              <div className="relative group">
                ...
                <button onClick={handleOpenEditProfileModal} className="absolute inset-0 w-full h-full bg-black/50 rounded-full flex items-center justify-center text-white text-2xl opacity-0 group-hover:opacity-100 transition-opacity" aria-label="Edit Avatar" disabled={loading}><FontAwesomeIcon icon={faEdit} /></button>
              </div>
              <div className="flex-grow">
                ...
                <div className="mt-4 space-x-3">
                    <button disabled={loading} onClick={handleOpenEditProfileModal} className="btn-secondary text-xs px-5 py-2 rounded-lg inline-flex items-center" ><FontAwesomeIcon icon={faEdit} className="mr-1.5"/> Edit Profil</button>
                </div>
              </div>
            </div>
          </div>
          <div className="card rounded-xl p-6 md:p-8 shadow-xl">
            <h3 className="text-xl md:text-2xl font-semibold mb-5 text-primary border-b border-white/10 pb-3 flex items-center"><FontAwesomeIcon icon={faRobot} className="mr-2.5" /> Statistik Saya</h3>
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4 md:gap-5 text-sm">
              {[
                { label: "Poin Quest", value: currentUser.stats?.points || 0 },
                { label: "Airdrop Diklaim", value: currentUser.stats?.airdropsClaimed || 0 },
                { label: "NFT Dimiliki", value: currentUser.stats?.nftsOwned || 0 },
                { label: "Garapan Aktif", value: activeAirdropsCount }
          ...
          <div className="mt-6 md:mt-8"> 
            <button disabled={loading} onClick={handleLogout} className="w-full btn-danger bg-red-600/80 hover:bg-red-700/90 text-white font-semibold py-3.5 px-6 rounded-lg flex items-center justify-center text-base"> 
              {loading ? <FontAwesomeIcon icon={faSpinner} spin className="mr-2.5" /> : <FontAwesomeIcon icon={faSignOutAlt} className="mr-2.5" />} Keluar 
            </button> 
          </div>
        </>
      )}
        ...
        <div className="fixed inset-0 bg-black/70 backdrop-blur-sm flex items-center justify-center p-4 z-[100]">
          <div className="modal-content card rounded-xl p-6 md:p-8 shadow-2xl w-full max-w-lg">
            <div className="flex justify-between items-center mb-6">
              <h3 className="text-2xl font-semibold text-white flex items-center"><FontAwesomeIcon icon={faEdit} className="mr-3 text-primary" /> Edit Profil</h3>
            ...
            <form onSubmit={handleUpdateProfile} className="space-y-5">
              <InputField parentLoading={loading} id="editName" label="Nama Tampilan / Username" value={editName} onChange={(e) => setEditName(e.target.value)} icon={faUser} />
              <InputField parentLoading={loading} id="editAvatarUrl" label="URL Avatar" value={editAvatarUrl} onChange={(e) => setEditAvatarUrl(e.target.value)} icon={faImage} />
              <div className="flex justify-end gap-4 pt-4">
                <button disabled={loading} type="button" onClick={handleCloseEditProfileModal} className="btn-secondary px-6 py-2.5 rounded-lg text-sm">Batal</button>
                <button disabled={loading} type="submit" className="btn-primary text-white px-6 py-2.5 rounded-lg text-sm flex items-center">
                  {loading ? <FontAwesomeIcon icon={faSpinner} spin className="mr-2" /> : <FontAwesomeIcon icon={faSave} className="mr-2" />} Simpan Perubahan
                </button>
              </div>
            </form>
          </div>
