...
import { 
  ...
  faParachuteBox, 
  faStore,
  faGem // Import icon permata untuk Quest
...
export default function BottomNav({ currentUser, hasNewAirdropNotification }) {
  ...
  const allNavItems = [
    ...
    // --- TAMBAHAN MENU QUEST ---
    { to: '/quests', icon: faGem, label_id: 'Misi', label_en: 'Quests', isPrivate: false }, 
  ...
  // Update logika grid untuk menampung 6 item (atau 5, 4, 3 sesuai kondisi)
  const gridColsClass = navItems.length === 6 ? 'grid-cols-6' 
          ...
          >
            {({ isActive }) => (
              <>
                ...
                <span 
                  className={`text-[10px] sm:text-xs mt-1 font-semibold transition-colors duration-200 truncate w-full px-1
